...
WmiQueryResult getWmiQueryResult(std::wstring wmiQuery, std::wstring propNameOfResultObject, bool allowEmptyItems = false) {
    ...
    if (FAILED(hres))
    {
        ...
        retVal.ErrorDescription = xorstr_(L"Failed to initialize COM library. Error code : ") + std::to_wstring(hres);
    }
    else
    {
        // Step 2: --------------------------------------------------
        ...
        if (FAILED(hres))
        {
            ...
            retVal.ErrorDescription = xorstr_(L"Failed to initialize security. Error code : ") + std::to_wstring(hres);
        }
        else
        {
            // Step 3: ---------------------------------------------------
            ...
            if (FAILED(hres))
            {
                ...
                retVal.ErrorDescription = xorstr_(L"Failed to create IWbemLocator object. Error code : ") + std::to_wstring(hres);
            }
            else
            {
                // Step 4: -----------------------------------------------------
                ...
                hres = pLoc->ConnectServer(
                    _bstr_t(xorstr_(L"ROOT\\CIMV2")), // Object path of WMI namespace
                ...
                if (FAILED(hres))
                {
                    ...
                    retVal.ErrorDescription = xorstr_(L"Could not connect to Wbem service.. Error code : ") + std::to_wstring(hres);
                }
                else
                {
                    // Step 5: --------------------------------------------------
                    ...
                    if (FAILED(hres))
                    {
                        ...
                        retVal.ErrorDescription = xorstr_(L"Could not set proxy blanket. Error code : ") + std::to_wstring(hres);
                    }
                    else
                    {
                        // Step 6: --------------------------------------------------
                        ...
                        if (FAILED(hres))
                        {
                            ...
                            retVal.ErrorDescription = xorstr_(L"Bad query. Error code : ") + std::to_wstring(hres);
                        }
                        else
                        {
                            // Step 7: -------------------------------------------------
                            ...
                            while (pEnumerator)
                            {
                                ...
                                if (S_OK != hr) {
                                    ...
                                    retVal.ErrorDescription = xorstr_(L"Couldn't extract property: ") + propNameOfResultObject + xorstr_(L" from result of query. Error code : ") + std::to_wstring(hr);
                                }
                                else {
...
std::wstring queryAndPrintResult(std::wstring query, std::wstring propNameOfResultObject)
{
    ...
    if (res.Error != WmiQueryError::None) {
        std::wcout << xorstr_("Got this error while executing query: ") << std::endl;
        std::wcout << res.ErrorDescription << std::endl;
        return L""; // Exitting function
    }

    return res.ResultList.at(0);
}

std::string hwid::get_hwid() {
    // Get serial number of Hard Drive
    std::wstring ws_hdd_serial = queryAndPrintResult(xorstr_(L"SELECT SerialNumber FROM Win32_PhysicalMedia"), xorstr_(L"SerialNumber"));

    // Get id of CPU
    std::wstring ws_cpu_id = queryAndPrintResult(xorstr_(L"SELECT ProcessorId FROM Win32_Processor"), xorstr_(L"ProcessorId"));

    std::string hdd_serial{ ws_hdd_serial.begin(), ws_hdd_serial.end() };
    std::string cpu_id{ ws_cpu_id.begin(), ws_cpu_id.end() };

    std::string hwid{ hdd_serial + cpu_id };

    SHA256 sha;
    sha.update(hwid);
    uint8_t* digest = sha.digest();
    std::string digest_string = SHA256::toString(digest);
    delete[] digest;
